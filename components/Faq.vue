<template>
  <section id="faq" class="py-28 scroll-mt-2">
    <div class="container m-auto px-4 max-w-4xl">
      ...
      <div class="mt-16">
        <ul>
          <li v-for="item in realData.data" :key="item" @click="OpenBox(item)" class="border-2 border-basic rounded-md px-4 py-4 lg:py-4 max-h-full mt-6 lg:mb-4">
            ...
            <div class="overflow-hidden max-h-0 transition-all duration-300 ml-[26px]"
              ...
              <p class="text-basic font-extralight text-base mt-1">{{ item.info }}</p>
            </div>
          </li>
        </ul>
        <div>
          ...
          <div class="text-center lg:flex lg:items-center lg:justify-center">
            ...
            <a class="inline-block mt-7 text-basic text-lg border border-basic py-3 px-6 rounded-md hover:bg-basic hover:text-white transition-all" :href="`mailto:` + realData.info2[0].email">Email us</a>
          </div>
        </div>
      </div>
    </div>
